...
function Services() {
  ...
  return (
    <div className="w-full py-16 bg-amber-200 text-black text-center flex justify-center items-center flex-col">
      ...
      />
      <button
        className="bg-zinc-900 text-white m-4 p-2 rounded-md"
        onClick={addTodo}
      >
        Add Services
      </button>
      ...
      <input
        ...
        type="text"
        value={sinp}
        onChange={(e) => setSinp(e.target.value)}
      />
      ...
      <input
        ...
        type="text"
        value={dInp}
        onChange={(e) => sdInp(e.target.value)}
      />
      <button onClick={() => { if (dInp.trim() != '') { setDesc(dInp) } }}>
        Change description
      </button>
      ...
      <input
        ...
        type="text"
        value={nInp}
        onChange={(e) => sNinp(e.target.value)}
      />
      <button onClick={() => {
        if (nInp.trim() != '') {
          setNotes(nInp)
      ...
      <input
        onChange={(e) => { setiDiff(e.target.value) }}
        type="text"
        value={idiff}
        placeholder="change difficult level"
      />
      <button onClick={() => { setProject({ ...project, difficulty: idiff }) }}>
        set difficulty
      </button>
      <button
        onClick={handleProj}
        className="border-2 bg-amber-400 font-bold text-sm p-1 rounded-sm text-white"
      >
        Set Project Details
      </button>
    </div>
